...
# > 컬럼 별칭(alias) 사용
SELECT name AS 학생명, grade AS 학년 FROM student;
...
# > IN : 여러 값 조건
SELECT * FROM student WHERE major IN ('컴퓨터공학', '경영학');
...
# > 전공별 평균 학년
SELECT major, AVG(grade) FROM student GROUP BY major;
-- 전공별 평균 학년 계산
...
UPDATE student SET grade = 2, major = '경제학' WHERE id = '2024001';
...
# > 학생 + 전공 테이블 조인
SELECT s.name, m.major_name FROM student s 
